module DoubleLevelStackApiModelConcern
  ...
  included do
    ...
    def self.process_json_items(items, site_id)
      ...
      self.transaction do
        items.each do |item|
          submodel_object = submodel_class_const.find_model_object(item[submodel] || item)
          base_model_object = base_model_class_const.find_model_object(item[base_model] || item)

          # Data are not present in the Databse, base_model is under higher scope then site --- impossible to detect like a human
          unless submodel_object
            submodel_data = (item[submodel] || item).select { |key, _| submodel_class_const::API_ATTRIBUTES.include? key }
            optional_data = {user_profiled: true}
            optional_data[:external_id] = (item[submodel] || item)[submodel_identifier] unless submodel_class_const::NOT_VALIDATE_EXTERNAL.include? submodel
            optional_data[:site_id] = site_id unless submodel_class_const::NOT_VALIDATE_SITE.include? submodel

            submodel_object = submodel_class_const.create!(submodel_data.merge(optional_data))
          end
